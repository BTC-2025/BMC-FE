...
deal1 = convert_lead_to_deal(db, lead_id=leads[0].id, title="Enterprise Software License", value=100000, performed_by=1, tenant_id=1)
deal2 = convert_lead_to_deal(db, lead_id=leads[1].id, title="Consulting Retainer", value=50000, performed_by=1, tenant_id=1)

print(f"✅ Converted 2 leads to deals")

# 3. Set deal stages
# deal1: Move to PROPOSAL
update_deal_stage(db, deal_id=deal1.id, new_stage="PROPOSAL", performed_by=1, tenant_id=1)
...
update_deal_stage(db, deal_id=deal2.id, new_stage="PROPOSAL", performed_by=1, tenant_id=1)
update_deal_stage(db, deal_id=deal2.id, new_stage="NEGOTIATION", performed_by=1, tenant_id=1)
update_deal_stage(db, deal_id=deal2.id, new_stage="WON", performed_by=1, tenant_id=1)
