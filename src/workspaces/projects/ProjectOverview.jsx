...
export default function ProjectOverview({ onSelectView, projects = [], tasks = [], milestones = [] }) {
  const activeMilestonesCount = milestones.filter(m => m.status === 'In Progress' || m.status === 'Upcoming').length;
  const completedTasksCount = tasks.filter(t => t.status === 'Completed').length;
  const taskProgress = tasks.length > 0 ? Math.round((completedTasksCount / tasks.length) * 100) : 0;
  
  const kpis = [
    { id: "Milestones", label: "Active Milestones", value: activeMilestonesCount.toString(), icon: "🚩", color: "bg-blue-50 text-blue-600", trend: "On Schedule" },
    { id: "Tasks", label: "Task Completion", value: `${taskProgress}%`, icon: "⚡", color: "bg-emerald-50 text-emerald-600", trend: "High Efficiency" },
    { id: "Projects", label: "Active Projects", value: projects.length.toString(), icon: "📈", color: "bg-purple-50 text-purple-600", trend: "Positive Yield" },
  ];
  ...
  const upcomingMilestone = milestones.find(m => m.status !== 'Completed') || milestones[0];
    ...
    <div className="p-8 md:p-12 max-w-[1700px] mx-auto space-y-12 animate-in fade-in duration-700 text-left">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 text-left">
          ...
          <div className="lg:col-span-2 space-y-8 text-left">
              ...
              >
                  {displayProjects.length > 0 ? displayProjects.map((proj, idx) => (
                    ...
                    <div className="text-center text-gray-400 font-bold uppercase tracking-widest">No active projects</div>
                  )}
                ...
                {upcomingMilestone ? (
                  <>
                    <div className="relative z-10 text-left">
                       ...
                       <h4 className="text-3xl font-[1000] tracking-tighter mb-4 leading-tight">{upcomingMilestone.title}</h4>
                       <p className="text-[11px] font-bold text-gray-400 max-w-[220px] leading-relaxed uppercase tracking-[0.1em]">{upcomingMilestone.description}</p>
                    </div>
                    <div className="relative z-10 text-left">
                        <div className="inline-flex items-center gap-3 px-5 py-3 bg-white/5 border border-white/10 rounded-2xl mb-8">
                           <span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse"></span>
                           <span className="text-[10px] font-black uppercase tracking-widest">Due: {upcomingMilestone.date}</span>
                        </div>
                        <button 
                  ...
                  <div className="relative z-10 flex flex-col items-center justify-center h-full text-center">
                    <h4 className="text-xl font-[1000] tracking-tighter text-gray-500 uppercase">No Upcoming Milestones</h4>
                  </div>
                )}
                {/* Back-glow */}
